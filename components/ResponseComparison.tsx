...
const ResponseComparison = ({ onClose }: ResponseComparisonProps) => {
  const { history } = useRequestStore()
  const [selectedResponses, setSelectedResponses] = useState<ResponseWithMeta[]>([])
  const [searchTerms, setSearchTerms] = useState(['', ''])

  const recentResponses: ResponseWithMeta[] = history.slice(0, 10).map(item => ({
    ...item.response,
    timestamp: item.timestamp,
    requestName: item.name || 'Unnamed Request'
  }))

  const addToComparison = (response: ResponseWithMeta) => {
  ...
  const removeFromComparison = (timestamp: number) => {
